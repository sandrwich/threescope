...
/** Satellite metadata decoded from the compact format. */
...
/** Transmitter entry decoded from the compact format. */
...
/** Get the raw data object, or null if not yet loaded. */
function getData(): Record<string, RawEntry> | null {
  return getSatnogsRaw() as Record<string, RawEntry> | null;
}
...
/** Look up SatNOGS data for a satellite by NORAD ID. Returns null if not in database or not loaded. */
...
let _searchIndexVersion: object | null = null; // track data identity to invalidate
...
export function getSatnogsSearchIndex(): SatnogsSearchEntry[] {
  const data = getData();
  if (!data) return [];
  // Invalidate if underlying data changed (e.g. fresh fetch replaced cached)
  if (_searchIndex && _searchIndexVersion === data) return _searchIndex;
  _searchIndex = [];
  _searchIndexVersion = data;
